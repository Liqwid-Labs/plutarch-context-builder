...
conditionalGenerator :: Gen ([SpendingBuilderElement], ValidatorUTXO Integer, ExpectedResult)
conditionalGenerator = do
    (elms, vutxo) <- genBuilderElements
    elements
        [ (inputFromSelf : elms, vutxo, Shouldn'tBuild)
        , (elms, vutxo, ShouldBuild)
        ]
  where
    inputFromSelf = InputFromOtherScript validatorHash mempty 0

conditionalShrinker ::
    ([SpendingBuilderElement], ValidatorUTXO Integer, ExpectedResult) ->
    [([SpendingBuilderElement], ValidatorUTXO Integer, ExpectedResult)]
conditionalShrinker (elms, vutxo, expected) = (,vutxo,expected) <$> shrink elms

spendingBuilderProperty :: Property
spendingBuilderProperty = forAllShrink conditionalGenerator conditionalShrinker go
  where
    go (elms, vutxo, tag) = checkCoverage $
        cover 50.0 (tag == Shouldn'tBuild) "shouldn't pass" $
            cover 50.0 (tag == ShouldBuild) "should pass" $
                case tag of
                    ShouldBuild ->
                        case ctx of
                            Just (rules -> f) -> f (elms, vutxo)
                            Nothing -> property False
                    Shouldn'tBuild -> property $ isNothing ctx
      where
        ctx = buildContext (elms, vutxo)
